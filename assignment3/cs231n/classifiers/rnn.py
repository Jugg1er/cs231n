...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=='rnn': 
            h,rnn_cache=rnn_forward(embed_out,h0,Wx,Wh,b)
        elif self.cell_type=='lstm':
            h,rnn_cache=lstm_forward(embed_out,h0,Wx,Wh,b)
        else:
            pass
        ...
        dh,dW_vocab,db_vocab=temporal_affine_backward(dx,temporal_affine_cache)
        if self.cell_type=='rnn':
            dembed_out,dh0,dWx,dWh,db=rnn_backward(dh,rnn_cache)
        elif self.cell_type=='lstm':
            dembed_out,dh0,dWx,dWh,db=lstm_backward(dh,rnn_cache)
        else:
            pass 
        _,dW_proj,db_proj=affine_backward(dh0,aff_cache)
