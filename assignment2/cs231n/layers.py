...
def relu_backward(dout, cache):
    ...
    Input:
    - dout:  derivUpstreamatives, of any shape
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        x_norm=(x-np.mean(x,axis=0,keepdims=True))/np.sqrt(np.var(x,axis=0,keepdims=True)+eps)
        out=gamma*x_norm+beta
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    N=x.shape[0]
    dmean1=np.sum(dx_norm*(-1)/np.sqrt(var+eps),axis=0,keepdims=True)
    dmean2=dvar*(-2)/N*np.sum(x-mean,axis=0,keepdims=True)
    dmean=dmean1+dmean2
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask=(np.random.rand(*x.shape)<p)/p     #result calculated with only p/1 neurons 
        out=x*mask                              #so the actual result should be result/p
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N,C,HX,WX=x.shape
    F,C,HF,WF=w.shape
    assert (HX-HF+2*pad)%stride==0
    assert (WX-WF+2*pad)%stride==0
    HO=int((HX-HF+2*pad)/stride+1)
    WO=int((WX-WF+2*pad)/stride+1)
    ...
    for i in range(N):
        for j in range(F):
            for r in range(HO):
                for c in range(WO):
                    out[i,j,r,c]=np.sum(w[j,:,:,:]*x_pad[i,:,r*stride:r*stride+HF,c*stride:c*stride+WF])+b[j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (HX-pool_height)%stride==0
    assert (WX-pool_width)%stride==0
    HO=int((HX-pool_height)/stride+1)
    WO=int((WX-pool_width)/stride+1)
    out=np.zeros((N,C,HO,WO))
    for i in range(N):
        for j in range(C):
            for r in range(HO):
                for c in range(WO):
                    out[i,j,r,c]=np.max(x[i,j,r*stride:r*stride+pool_height,c*stride:c*stride+pool_width])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for r in range(HO):
                for c in range(WO):
                    x_window=x[i,j,r*stride:r*stride+pool_height,c*stride:c*stride+pool_width]
                    x_max=np.zeros_like(x_window)
                    index=np.argmax(x_window)
                    x_max[int(index/pool_width),index%pool_width]=1
                    dx[i,j,r*stride:r*stride+pool_height,c*stride:c*stride+pool_width]+=dout[i,j,r,c]*x_max
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dmean2_var = dvar * -2.0 / M * np.sum(x_group - mean, axis=(2,3,4), keepdims=True)
    dmean = dmean1 + dmean2_var
    dx_group1 = dx_groupnorm * 1.0 / np.sqrt(var + eps)
    dx_group2_mean = dmean * 1.0 / M
    dx_group3_var = dvar * 2.0 / M * (x_group - mean)
